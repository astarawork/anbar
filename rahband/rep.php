...
// تابع بررسی وجود پلاک در جدول rghabz با act=1
...
<head>
    ...
    <style>
        ...
        .copy-column-header:hover::after {
            opacity: 1;
        }
        .copy-status-header {
            position: relative;
        ...
        .copy-status-header:hover::after {
            opacity: 1;
        }
        .column-highlight {
...
<div class="container">
    ...
    // ایجاد آرایه برای ذخیره مختصات پلاک‌ها و وضعیت رها شدن
    ...
    <?php
    echo "زمان بارگذاری اطلاعات راهبند : ".$tarom;
    ?>
                        ...
                        <tr>
                            ...
                            <td class="status-cell copy-status-column">
                                ...
                                if($countf>0){
                                    ...
                                    if($ekht > $rib){
                                        ...
                                        // بررسی وجود پلاک در rghabz با act=1
                            ...
                            <td>
                                <?php if(isset($rkarbar_ids[$pelak])): ?>
                                    <a href="edit_rkarbar.php?id=<?php echo $rkarbar_ids[$pelak]; ?>" class="edit-btn" title="ویرایش">
                            ...
                            <td class="status-cell copy-status-column">
                                ...
                                if($countf>0){
                                    ...
                                    if($ekht > $rib){
                                        ...
                                        if(!$is_in_rghabz) {
                                            echo '<button class="renew-btn" onclick="renewPelak(\''.$pelak.'\')">
                                                  <i class="fas fa-redo me-1"></i>تمدید
                                                  </button>';
                                        }
                                    } else {
                                        echo '<span class="badge badge-valid"><i class="fas fa-check-circle me-1"></i>معتبر</span>';
...
<script>
    ...
    function showOnMap(pelak, lat, lng) {
        ...
        document.getElementById('location-info').innerHTML = `
            <strong>عرض جغرافیایی:</strong> ${lat.toFixed(6)} <br>
            <strong>طول جغرافیایی:</strong> ${lng.toFixed(6)}
        `;
        ...
        setTimeout(() => {
            ...
            const marker = L.marker([lat, lng]).addTo(window.map)
                .bindPopup(`
                    <b>پلاک:</b> ${pelak}<br>
                    <b>مختصات:</b> ${lat.toFixed(6)}, ${lng.toFixed(6)}
                `)
                .openPopup();
        }, 500);
    }
    ...
    function copyColumn(columnIndex) {
        ...
        rows.forEach(row => {
            ...
            if (cells.length > columnIndex) {
                ...
                // حالت خاص برای ستون وضعیت قبض انبار
                if (columnIndex === 2) {
                ...
                // سایر ستون‌ها
                    ...
                    const badges = cell.querySelectorAll('.badge');
                    badges.forEach(badge => {
                        const textNode = document.createTextNode(badge.textContent.trim());
                        badge.parentNode.replaceChild(textNode, badge);
                    });
                    const links = cell.querySelectorAll('.pelak-link');
                    links.forEach(link => {
        ...
        navigator.clipboard.writeText(columnContent.trim()).then(() => {
            ...
            if (headerCells[columnIndex]) {
                ...
                setTimeout(() => {
                    headerCells[columnIndex].classList.remove('column-highlight');
                }, 1000);
            }
    ...
    function showToast(message, type = 'success') {
        ...
        toast.innerHTML = `
            <div class="toast-header ${bgClass} text-white">
                <strong class="me-auto">اطلاع رسانی</strong>
    ...
    // افزودن رویداد کلیک برای ستون‌های قابل کپی
    document.addEventListener('DOMContentLoaded', function() {
        const copyColumnHeaders = document.querySelectorAll('.copy-column-header, .copy-status-header');
        
        copyColumnHeaders.forEach((header, index) => {
